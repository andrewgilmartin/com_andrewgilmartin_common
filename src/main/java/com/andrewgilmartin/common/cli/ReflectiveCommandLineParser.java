 ...
 * is licensed under a Creative Commons Attribution 3.0 Unported License. For
 * more information see http://creativecommons.org/licenses/by/3.0/
 */
import com.andrewgilmartin.common.exceptions.CommonIllegalArgumentException;
 ...
 * the command line option "--verbose" is found, the method "void setRe( Pattern
 * pattern )" is called when "--re 'foo.*bar'" is found, and "void setDatabase(
 * URI jdbcUri, String user, String password )" is called when "--database
 * jdbc:mysql://localhost/xx root r00t" is found. Note that the method names can
 * begin with "set" or "add", eg "addDatabase".
 *
 ...
 * textual form to their object form. For the common types this class knows how
 * to convert the value from text to its typed instance. The common types are
 *
 * <ul>
 * <li>java.io.File
...
public class ReflectiveCommandLineParser {
     ...
     */
    public void run(Runnable tool, String[] arguments) throws CommonIllegalArgumentException {
    ...
    /**
     * Parse the given arguments against the methods of the given tool. Start
     * parsing the arguments array at the given offset. Once parsed run the
     * tool.
     */
    public void run(Runnable tool, String[] arguments, int argumentsOffset) throws CommonIllegalArgumentException {
     ...
     */
    public void parse(Object tool, String[] arguments, int argumentsOffset) throws CommonIllegalArgumentException {
                ...
                if (optionRunner != null) {
                    ...
                    if (optionRunner.getName().startsWith("set")) {
                        if (usedSetters.contains(optionRunner)) {
                            throw new CommonIllegalArgumentException("command line option '--{0}' must only be used once", optionName);
                        }
                    ...
                    if (optionRunner.getMethod().getParameterTypes().length == 0) {
                        ...
                        throw new CommonIllegalArgumentException("command line option '--{0}' has too few arguments", optionName);
                    }
                } else {
                    throw new CommonIllegalArgumentException("unknown command line option '--{0}'", optionName);
                }
            } else {
            ...
            while (argumentIndex < arguments.length) {
                ...
                if (positionalRunner.getName().startsWith("set")) {
                    if (usedSetters.contains(positionalRunner)) {
                        throw new CommonIllegalArgumentException("positional command line arguments must only be used once");
                    }
                ...
                if (argumentIndex + positionalRunner.getParameterTypes().length - 1 < arguments.length) {
                    ...
                    throw new CommonIllegalArgumentException("too few positional command line arguments");
                }
            }
        }
        // are all the arguments used?
        if (argumentIndex < arguments.length) {
            throw new CommonIllegalArgumentException("too many positional command line arguments");
        }
        // done
    }
     ...
     */
    protected MethodRunner getOptionMethodRunner(Object instance, String optionName) throws CommonIllegalArgumentException {
     ...
     */
    protected MethodRunner findMethodRunner(Object instance, String methodName) throws CommonIllegalArgumentException {
        while (instance != null) {
            ...
            try {
                ...
                throw new CommonIllegalStateException(e);
            }
        }
        return null;
    }
     ...
     */
    protected Object convertArgumentValue(Class type, String raw) throws CommonIllegalArgumentException {
        try {
            ...
            return null;
        } catch (ParseException | ClassNotFoundException | IllegalArgumentException e) {
            throw new CommonIllegalArgumentException(e);
        }
    }
}
